...
SEM is also referred to as a *Covariance Structure Analysis*

We see that these models has the following workflow:

1. Define the individual constructs
2. Develop measurement model
3. Design a study to produce empirical results
4. Measurement model validity
5. Specify SEM
6. SEM Validity

To go through that workflow we are going to apply three different tools:

1. EFA,
2. CFA, and
3. SEM

Where **EFA** aims to explore the data to get an idea of how many constructs we need, unless we already have a hypothesis hereof. Although the latent variables cannot be directly measured so we need a good foundation for these.

We see that the **CFA** aims to verify the relationships and validate the model. We need to meet a lot of criteria to get an optimal model, such as 

**Model fit:**
a. Information criteria such as TLI, CFI, RMSEA

**Construct validty**

b. Convergent validity: we look at loadings we want them to be higher than 50% and ideally 70%. We also look at item reliability, where we want to look at the AVE pr. factor and CR.
c. Discriminant validity: We want constructs to represent each own variables, hence we dont want to have too much of multicollinearity, e.g., if the correlation between two constructs is near 1, then we can just disregard one of them, as they just say the same
d. Nomological validity: That is if there is an actual relationship between constructs. The Matrix of construct correlations can be use for this.
e. Face validity: You must selection a metric that reflects the overall purpose, hence choosing the right scale to measure your overall model, this should be chosen prior to the study.

**Model diagnostics**

f. Standardized residuals: We want to see if the model is able to explain the observations. In general we want the standardized residuals to be lower than 2.5 and not higher than 4. The interval inbetween one should consider these observations, perhaps see what they are.

After all these checks we are going to *impose structure on the model*, that is when we are reaching SEM! When we impose structure we do:

1. Set directions between latent variables. In EFA and CFA these are merely bidirections.
2. Decision of recursive / nonrecursive model. We must consider whether we want a recursive model, where endogenous latent variables cannot be exogenous to each other, hence no feedback loops. On the other hand, if we allow for a nonrecursive model, we see that we have feedback loops and variance in one endogenous latent variable explains a lot of variance in another latent variable. Usually we want to avoid nonrecursive models, as the LV's will be explaining each other (hence can we just use one?) and also we need more identification constraints.


## Application with Lavann and HBAT data
...
• A 1st type of information is available from HBAT's data warehouse and includes information on:
...
communality(epsi) #These are just the loadings squared
```

Commonality must be higher than correlation between the latent variables. If not, you can argue that two constructs should be merged.
...
Example: if we increase EXP by 1 standard deviation, the VAL will increase by 0.036 standard deviations.
...
We see tat that cluster 1 appear to be different from the two other clusters. Therefore, we also see the Tukey HSF is also saying that there is difference between 1 - 2 and 1 - 3, although 2 - 3 are pretty much the same.
...
-   For clustering with categorical variables, one option is to use Expectation Maximization algorithm, which yields probabilistic
classes (clusters).
...
### An example with McDonalds data
...
We see that the categorical data is actually coded in words, this we need to transform.
...
Let us look at the colmeans.

```{r}
round(colMeans(MD.x), 2) %>% barplot(ylim = c(0,1),col = "darkgrey",main = "Variable means")
abline(h = seq(0,1,0.1),col = "darkgrey",lty = 2)
```

We see that few people rate the food to be spicy and most think that it is convenient, fast and fattening.
...
We see that the first three components reaches the 60% level.
...
We see component 1 is highly influenced by yummy and tasty, hence something about the taste.
...
Results indicate that some attributes are strognly related to one another and that price dimension may be critical in differentiating between groups of consumer
...
1. Global stability-based data structure analysis, make boxplots for each clusters through bootstrapping to see cluster stability/reporducability, and
2. Segment Stability, to assess the stability of given segments (clusters)
...
1. A big increase in heterogeneity. Although that is not visible.
...
The fundamental idea is to conduct a large number of repeated computations
...
it can be assumed that clusters exist in the data.

**The main steps**

1. draw bootstrap samples
2. compute the Rand index
3. inspect boxplots to assess the reproducibility of cluster solutions
...
plot(MD.b28, xlab = "number of segments",
     ...
     ,main = "Global Stability Plot")
```

See the distribution of stability for each number of segments.
...
Now we are looking at the segment stability for the 4 cluster model.
...
a) to compare the bars for each segment with the horizontal line to see what makes each segment distinct from all consumers in the market
b) compare the bars across segments to identify the difference between segments. 
...
+ segment 1 and 4 see the restaurant cheap, with
...
colored cells indicate the deviation of observed frequencies from the expected frequencies 
...
in my case, segment 2 contain significanty more men (blue box in the bottom of the second column) the rest have similar gender distribution (all white boxes)
...
the plot suggests significant differences in average age across segments
...
We see that we have computed the mean for each cluster. Where cluster 4 is the lowest while cluster 1 the highest.

*Doing the same, but for Like*

```{r}
table(mcdonalds$Like)
mcdonalds$Like <- as.factor(mcdonalds$Like)
...
We see the means again for each cluster.
...
female
# prop.table(table(mcdonalds$Gender, k4), 2)
```
